  ...
  with HasLoadHelper
{
  ...
  * */
  ...
  XSPerfAccumulate("util_busy_cycle", state =/= s_idle || io.in.valid || !isEmpty(enqPtr, deqPtr))
  XSPerfAccumulate("util_idle_cycle", state === s_idle && !io.in.valid && isEmpty(enqPtr, deqPtr))
  ...
  XSPerfAccumulate("util_dtlb_req_fire", io.dtlb.req.fire)
  XSPerfAccumulate("util_dtlb_req_stall", io.dtlb.req.valid && !io.dtlb.req.ready)
  XSPerfAccumulate("util_dtlb_resp_fire", io.dtlb.resp.fire)
  XSPerfAccumulate("util_dcache_req_fire", io.rdcache.req.fire)
  XSPerfAccumulate("util_dcache_req_stall", io.rdcache.req.valid && !io.rdcache.req.ready)
  XSPerfAccumulate("util_dcache_resp_fire", io.rdcache.resp.fire)
  XSPerfAccumulate("util_sbuffer_fire", io.sbuffer.fire)
  XSPerfAccumulate("util_sbuffer_stall", io.sbuffer.valid && !io.sbuffer.ready)
  XSPerfAccumulate("util_writeback_fire", io.uopwriteback.fire)
  XSPerfAccumulate("util_writeback_blocked", io.uopwriteback.valid && !io.uopwriteback.ready)
  XSPerfAccumulate("state_idle", state === s_idle)
  XSPerfAccumulate("state_flush_sbuffer_req", state === s_flush_sbuffer_req)
  XSPerfAccumulate("state_wait_flush_sbuffer_resp", state === s_wait_flush_sbuffer_resp)
  XSPerfAccumulate("state_tlb_req", state === s_tlb_req)
  XSPerfAccumulate("state_wait_tlb_resp", state === s_wait_tlb_resp)
  XSPerfAccumulate("state_pm", state === s_pm)
  XSPerfAccumulate("state_cache_req", state === s_cache_req)
  XSPerfAccumulate("state_cache_resp", state === s_cache_resp)
  XSPerfAccumulate("state_misalign_merge_data", state === s_misalign_merge_data)
  XSPerfAccumulate("state_latch_and_merge_data", state === s_latch_and_merge_data)
  XSPerfAccumulate("state_send_data", state === s_send_data)
  XSPerfAccumulate("state_wait_to_sbuffer", state === s_wait_to_sbuffer)
  XSPerfAccumulate("state_finish", state === s_finish)
  XSPerfAccumulate("state_fof_fix_vl", state === s_fof_fix_vl)
}
